...
trait AuthenticatesAndRegistersUsers
{
    ...
    {
        $this->validate($request, array('user' => 'required', 'password' => 'required'));
        $credentials = $request->only('user', 'password');
        if ($this->auth->attempt($credentials, $request->has('remember'))) {
            return redirect()->intended($this->redirectPath());
        }
        return redirect($this->loginPath())->withInput($request->only('user', 'remember'))->withErrors(array('user' => $this->getFailedLoginMessage()));
    }
